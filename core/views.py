...
def staff_attendance_list(request):
    ...
    if selected_date_str:
        try:
            ...
            # Fallback to default if date string is invalid
...
def approve_attendance_batch(request, batch_id):
    ...
    if request.method == 'POST':
        with transaction.atomic():
            ...
            active_sy = SchoolYear.objects.filter(is_active=True).first()
            offense_cache = {o.code: o for o in Offense.objects.all()}
            for rec in batch.records.select_related('student').all():
                sync_student_attendance_logs(rec.student, batch.daily_attendance, request.user, active_sy, offense_cache)
...
@login_required
def reopen_attendance_batch(request, batch_id):
    if not request.user.is_staff: return redirect('home')
    batch = get_object_or_404(PeriodAttendance, id=batch_id)
    ...
    if request.method == 'POST':
        with transaction.atomic():
            ...
            # 2. Trigger Sync to delete old auto-generated records for this day
            # Since is_approved is False now, the sync will see fewer approved records
            # and automatically clean up any offenses generated by this period
            active_sy = SchoolYear.objects.filter(is_active=True).first()
            offense_cache = {o.code: o for o in Offense.objects.all()}
            for rec in batch.records.select_related('student').all():
                sync_student_attendance_logs(rec.student, batch.daily_attendance, request.user, active_sy, offense_cache)

            messages.warning(request, f"⚠️ Period {batch.period_number} has been reopened for editing.")
...
def sync_student_attendance_logs(student, daily_attendance, staff_user, active_sy=None, offense_cache=None):
    """
    ...
    with transaction.atomic():
        ...
        if active_sy is None:
            active_sy = SchoolYear.objects.filter(is_active=True).first()
        if offense_cache is None:
            offense_cache = {o.code: o for o in Offense.objects.all()}
        ...
        for o_code in offenses_to_log:
            official_offense = offense_cache.get(o_code)
    ...
    if request.method == 'POST':
        try:
            ...
            for sig_type in ['father', 'mother', 'guardian']:
                ...
                if sig_b64:
                    if hasattr(student, f'{sig_type}_signature'):
                        sig_field = getattr(student, f'{sig_type}_signature')
                        if sig_b64 == 'DELETE':
                            if sig_field and sig_field.name:
                                sig_field.delete(save=False)
                            ...
                            if sig_field and sig_field.name:
                                sig_field.delete(save=False) # Delete old file
                            sig_field.save(filename, ContentFile(base64.b64decode(imgstr)), save=False)
            if photo_b64:
                if photo_b64 == 'DELETE':
...
def staff_encode_attendance(request, section_id, date_str):
        ...
        with transaction.atomic():
            ...
            if not period_att.is_approved:
                ...
                for rec in period_att.records.select_related('student').all():
                    sync_student_attendance_logs(rec.student, daily_att, request.user, active_sy, offense_cache)
                ...
                messages.error(request, f"Period {period_number} is already locked and approved in the system.")
    ...
    submitted_period_numbers = [p.period_number for p in today_periods if p.is_approved]
    ...
    context.update({
        ...
        'period_batch_ids': json.dumps({p.period_number: p.id for p in today_periods}),
    })
